...
float stepFactor = 1; //should be tuned

void VIratio_dynamicStep_optimize_load() {
	...
	if (load_Optimize_Enable) {
		if (L_Power > regulate_Power_Setpoint) {
			...
			if(L_Current - (VIratio * L_Voltage) < 0){						//if load current needs to be increased
				DAC_Voltage = DAC_Voltage - (L_Current - (VIratio * L_Voltage)) * stepFactor; //increased DAC voltage by an amount proportional to the distance from desired ratio
			}
			else if(L_Current - (VIratio * L_Voltage) > 0){					//if load current needs to be decreased
				DAC_Voltage = DAC_Voltage + (L_Current - (VIratio * L_Voltage)) * stepFactor; //decrease DAC voltage by an amount proportional to the distance from desired ratio
			}
		}
	}
}
